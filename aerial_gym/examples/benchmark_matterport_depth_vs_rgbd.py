...
def _print_table(depth_rows, rgbd_rows):
    ...
    print(
        f"{'envs':>6} | {'depth FPS':>12} | {'rgbd FPS':>12} | {'rgbd/depth':>11} | "
        f"{'depth RTF':>12} | {'rgbd RTF':>12}"
    )
    print("-" * 82)
    for d, r in zip(depth_rows, rgbd_rows):
        ratio = r["fps"] / max(d["fps"], 1.0e-9)
        print(
            f"{d['num_envs']:6d} | {d['fps']:12.2f} | {r['fps']:12.2f} | {ratio:11.3f} | "
            f"{d['real_time_speedup']:12.2f} | {r['real_time_speedup']:12.2f}"
        )
